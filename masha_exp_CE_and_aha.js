...
async function experimentInit() {
  ...
  aha_scale_eval_text = new visual.TextStim({
    ...
    text: 'Оцените его интенсивность от 1 до 7, кликнув мышкой: где 1 – «ощущение было слабо выраженным», 7 – «ощущение было ярко выраженным». Если ага-переживания не было нажмите «0»',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function instr_before_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instr_before_trainComponents)
...
function CRA_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CRA_trainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CRA_ans_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CRA_ans_trainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function is_aha_trRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of is_aha_trComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function scale_aha_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    text_aha_train.setText('Оцените его интенсивность от 1 до 7, кликнув мышкой: где 1 – «ощущение было слабо выраженным», 7 – «ощущение было ярко выраженным». Если ага-переживания не было нажмите «0»');
    ...
    for (const thisComponent of scale_aha_trainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CE_trainRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CE_trainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instr_after_trRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instr_after_trComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CRA_3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CRA_3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CRA_ansRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CRA_ansComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function is_ahaRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of is_ahaComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function scale_ahaRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of scale_ahaComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CE_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of CE_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function conclusionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of conclusionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
